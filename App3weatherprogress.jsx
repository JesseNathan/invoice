...
let Weather = {
    ...
    fetch(`${weatherUrl.baseUrl}key=${weatherUrl.key}&q=${city}`)
    .then((response) => {
        if(!response.ok){
            console.log("can't detect Weather")
        }
...
geocodeTranslate: function (lat, long){
    const keyLocation = "ddaff123dad14c459ff2688713748312";
    const urlLocation = "https://api.opencagedata.com/geocode/v1/json?"
    const GeocodeApi = urlLocation + "key=" + keyLocation + "&q=" + lat + "+" + long + "&pretty=1" + "&no_annotations=1"

    const requestLocation = new XMLHttpRequest();

    requestLocation.open("GET", GeocodeApi, true);

    requestLocation.onload = function(){
        if (requestLocation.status == 200){
            const data = JSON.parse(requestLocation.response)
        }
    }
